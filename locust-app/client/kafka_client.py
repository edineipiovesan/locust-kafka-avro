...
class KafkaClient:
    ...
    def __handle_failure(self, *arguments, **kwargs):
        ...
        request_data = dict(request_type="ENQUEUE", name=kwargs["topic"], response_time=elapsed_time,
                            response_length=len(message) if message is not None else -1, exception=arguments[0])
    ...
    def send(self, topic, key=None, message=None):
        ...
        future.add_errback(self.__handle_failure, start_time=start_time, message=message, topic=topic)

    def finalize(self):
        print("flushing the messages")
        self.producer.flush(timeout=180)
        self.producer.close(timeout=180)
